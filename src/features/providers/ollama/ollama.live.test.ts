...
const LIVE = process.env.code_LIVE_TEST === "1" && process.env.code_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.code_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.code_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.code_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.code_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.code_LIVE_OLLAMA_WEB_SEARCH !== "0";
...
async function withTempcodeState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "code-ollama-cli-live-"));
  try {
    await fs.writeFile(
...
async function runcodeCli(args: string[], env: NodeJS.ProcessEnv) {
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "code-ollama-cli-output-"));
  ...
  try {
    const result = spawnSync(process.execPath, ["code.mjs", ...args], {
      cwd: process.cwd(),
      env,
      timeout: 90_000,
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  return {
    ...
    code_LIVE_TEST: "1",
    code_LIVE_OLLAMA: "1",
    code_LIVE_OLLAMA_WEB_SEARCH: "0",
    code_STATE_DIR: path.join(root, "state"),
    code_CONFIG_PATH: path.join(root, "code.json"),
    code_NO_RESPAWN: "1",
    code_TEST_FAST: "1",
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without PI model discovery", async () => {
    await withTempcodeState(async ({ root }) => {

...
import { resolvePluginConfigObject } from "code/plugin-sdk/plugin-config-runtime";
import {
  definePluginEntry,
  type codePluginApi,
...
import { buildApiKeyCredential } from "code/plugin-sdk/provider-auth";
import type {
...
export default definePluginEntry({
  ...
  register(api: codePluginApi) {
    ...
    const resolveCurrentPluginConfig = (config?: codeConfig): OllamaPluginConfig => {
    ...
    api.registerProvider({
      ...
      buildUnknownModelHint: () =>
        ...
        'Set OLLAMA_API_KEY="ollama-local" (any value works) or run "code configure". ' +
        "See: https://docs.code.ai/providers/ollama",
    });
  },
});

...
import type { ModelDefinitionConfig } from "code/plugin-sdk/provider-onboard";
import { withFetchPreconnect } from "code/plugin-sdk/test-env";
...
describe("Ollama provider", () => {
  const createAgentDir = () => mkdtempSync(join(tmpdir(), "code-test-"));
  ...
  async function runOllamaCatalog(params: { config?: codeConfig; env?: NodeJS.ProcessEnv }) {

...
type OllamaProviderConfigOverride = Partial<{
  ...
  baseURL: string;
  models: NonNullable<
    NonNullable<NonNullable<codeConfig["models"]>["providers"]>[string]
  >["models"];
}>;

function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): codeConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): codeConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    const result = await tool.execute({ query: "code docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://code.ai/docs" }],
    });
  ...
  it("tries the future local direct endpoint when the local proxy endpoint is missing", async () => {
    ...
    await expect(
      runOllamaWebSearch({ config: createOllamaConfig(), query: "code" }),
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest code release" })).rejects.toThrow(
      "ollama signin",
    );
  });

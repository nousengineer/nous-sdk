...
// All code/plugin-sdk module stubs are provided by test-preload.ts via bunfig.toml [test.preload].
// Override the ssrf-runtime mock per-test to control network calls in streaming tests.
const fetchWithSsrFGuardMock = vi.fn();

vi.mock("code/plugin-sdk/ssrf-runtime", () => ({
...
describe("buildOllamaChatRequest", () => {
  ...
  it("omits parallel_tool_calls from the Ollama payload", () => {
    expect(
      buildOllamaChatRequest({
        modelId: "qwen3.5:9b",
        messages: [{ role: "user", content: "hello" }],
        requestParams: { parallel_tool_calls: true, temperature: 0.2 },
      }),
    ).toEqual({
      model: "qwen3.5:9b",
      messages: [{ role: "user", content: "hello" }],
...
describe("convertToOllamaMessages", () => {
  ...
  it("handles tool_use blocks with string input (chronokairo format round-trip)", () => {
    const messages = [
      {
        role: "assistant",
        content: [
  ...
  it("strips thinking content blocks from assistant history in multi-turn conversations", () => {
    // After a turn where the model emitted a thinking block, the history entry
    // will have content: [{type:"thinking",...}, {type:"text",...}].
    // convertToOllamaMessages must only pass the text part back to Ollama.
    const messages = [
      {
        role: "assistant",
        content: [
          { type: "thinking", thinking: "Internal reasoning step" },
  ...
  it("strips thinking-only assistant history gracefully in multi-turn conversations", () => {
    // Edge case: if an assistant turn produced only a thinking block and no text
    const messages = [
      {
        role: "assistant",
        content: [{ type: "thinking", thinking: "Just thinking, no visible output" }],
      },
    ];
    const result = convertToOllamaMessages(messages);
    expect(result).toHaveLength(1);
...
describe("buildAssistantMessage", () => {
  ...
  it("strips unclosed thinking channel markers from visible content", () => {
    const response = {
      model: "qwen3:32b",
      created_at: "2026-01-01T00:00:00Z",
      message: {
  ...
  it("does not strip closed thinking channel markers (should be kept as-is)", () => {
    // When <channel|> is present, the model used the channel format correctly;
    // sanitizeThinkingBlock must leave this content untouched.
    const response = {
      model: "gemma4:31b",
      created_at: "2026-01-01T00:00:00Z",
      message: {
    ...
    const result = buildAssistantMessage(response, modelInfo);
    // Content is left as-is because the close tag is present

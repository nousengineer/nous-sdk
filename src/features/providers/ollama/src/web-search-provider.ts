import type { codeConfig } from "code/plugin-sdk/config-types";
import {
  isNonSecretApiKeyMarker,
  normalizeOptionalSecretInput,
} from "code/plugin-sdk/provider-auth";
import { resolveEnvApiKey } from "code/plugin-sdk/provider-auth-runtime";
import {
...
} from "code/plugin-sdk/provider-web-search";
import { fetchWithSsrFGuard } from "code/plugin-sdk/ssrf-runtime";
import { normalizeOptionalString } from "code/plugin-sdk/text-runtime";
...
function resolveConfiguredOllamaWebSearchApiKey(config?: codeConfig): string | undefined {
...
function resolveOllamaWebSearchApiKey(config?: codeConfig): string | undefined {
  return resolveConfiguredOllamaWebSearchApiKey(config) ?? resolveEnvOllamaWebSearchApiKey();
}

function resolveOllamaWebSearchBaseUrl(config?: codeConfig): string {
...
export function createOllamaWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    docsUrl: "https://docs.code.ai/tools/web",
    autoDetectOrder: 110,
    credentialPath: "",

import { CUSTOM_LOCAL_AUTH_MARKER } from "code/plugin-sdk/provider-auth";
import type { codeConfig } from "code/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "code/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "code/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "code/plugin-sdk/provider-setup";
import type { WizardPrompter } from "code/plugin-sdk/setup";
...
vi.mock("code/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("code/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("code/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("code/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): codeConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as codeConfig),
    env: params?.env ?? {},
...
describe("lmstudio setup", () => {
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("code_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
